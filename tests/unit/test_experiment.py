...
class TestExperiment:
    ...
    def test_add_pairwise_evaluation(self, experiment, pairwise_answer_evaluation):
        ...
        assert game.evaluations == {"pairwise": pairwise_answer_evaluation}
    ...
    def test_add_elo_tournament(self, experiment, elo_tournament_result):
        ...
        assert experiment.elo_tournaments == [elo_tournament_result]
    ...
    def test_add_documents_from_csv_reads_the_default_id_columns(self, empty_experiment):
        """Called directly the id columns default to `qid` and `did`; the constructor infers them instead."""
...
class TestExperimentSerialization:
    ...
    def test_convenience_properties_after_deserialization(self, retrieval_evaluation):
        loaded_result = RetrievalEvaluatorResult.model_validate_json(retrieval_evaluation.model_dump_json())
    ...
    def test_load_corrupted_jsonl_gracefully(self, tmp_path, base_experiment_config):
        ...
        # The valid result is loaded and the corrupted lines are skipped
        assert experiment["0"].retrieved_docs["0"].evaluations.keys() == {"reasoner"}
