...
from benchmarks.llmjudge import load
from benchmarks.run_llmjudge_pairwise import PreferencePair, judge, sample_pairs
...
class TestPairwisePreferences:
    def test_pairs_put_the_higher_graded_passage_of_a_query_first(self):
        assert sample_pairs(load(DATA_DIR, "test"), n_pairs=10) == [PreferencePair("q2", better="p2", worse="p3")]
    ...
    def test_accuracy_is_the_share_of_games_won_by_the_higher_graded_passage(self, llm_provider_mock, tmp_path):
        ...
        pairs = sample_pairs(data, n_pairs=10)
        evaluator = get_answer_evaluator("pairwise", llm_provider=llm_provider_mock)

        llm_provider_mock.async_call_mocker = prefers(data.passages["p2"])
        right = judge(data, pairs, evaluator, "right", tmp_path)
        llm_provider_mock.async_call_mocker = prefers(data.passages["p3"])
        wrong = judge(data, pairs, evaluator, "wrong", tmp_path)
    ...
    def test_a_tie_earns_half_a_point_in_every_breakdown(self, llm_provider_mock, tmp_path):
        ...
        tied = judge(data, sample_pairs(data, n_pairs=10), evaluator, "tied", tmp_path)
    ...
    def test_a_game_with_a_failed_answer_order_counts_as_failed_and_leaves_no_accuracy(
        ...
        found = judge(data, pairs, evaluator, "failed", tmp_path)
