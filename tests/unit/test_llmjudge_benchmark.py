...
from benchmarks.run_llmjudge import EVALUATORS, judge
...
class TestLLMJudgeRunner:
    @pytest.mark.parametrize("variant", list(EVALUATORS))
    def test_every_variant_in_the_grid_judges_through_an_experiment(self, variant, llm_provider_mock, tmp_path):
        ...
        qrels = judge(data, evaluator, f"test_{variant}_mock", tmp_path)

        assert qrels["q2"].keys() == data.qrels["q2"].keys()
    ...
    def test_judge_labels_every_pair_and_a_rerun_makes_no_llm_calls(self, llm_provider_mock_retrieval, tmp_path):
        ...
        qrels = judge(data, evaluator, "test_reasoner_mock", tmp_path)

        assert qrels == {"q2": {"p2": 2, "p3": 2}}
        assert llm_provider_mock_retrieval.async_call_mocker.call_count == 2

        assert judge(data, evaluator, "test_reasoner_mock", tmp_path) == qrels
        assert llm_provider_mock_retrieval.async_call_mocker.call_count == 2
