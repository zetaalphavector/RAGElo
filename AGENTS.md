...
`BaseLLMProvider` defines the interface: `call_async(input, response_schema) → LLMResponseType[T]`. Implementations: `OpenAIProvider` (structured output via Responses API), `VercelProvider` (the same client pointed at the Vercel AI Gateway), `OllamaProvider`, `InstructorProvider`, and the two `JevProvider`s, `TypeSafeProvider` and `VercelJevProvider`. Providers return `LLMResponseType[T]` with `raw_answer` and `parsed_answer`.

`JevProvider` is the base of the providers that reach TypeSafe's Jev: `TypeSafeProvider` (`"typesafe"`) through TypeSafe's SDK, an optional dependency (`pip install 'ragelo[typesafe]'`) imported the same way as instructor, and `VercelJevProvider` (`"vercel-jev"`) through the Vercel AI Gateway. Jev answers typed questions about a state with probabilities and generates no text, so it returns a `JevResponse` and only the evaluators built on `JevEvaluatorMixin` can use it. They put the questions on `LLMInputPrompt.questions` and turn the response into the usual answer formats in `_process_answer`.
