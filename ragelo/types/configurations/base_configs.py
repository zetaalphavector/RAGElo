...
class BaseConfig(BaseModel):
    ...
    data_dir: Optional[str] = Field(
        default=None,
        description=(
            "Path to the data folder. Defaults to the root folder where the script is being executed. "
            "Every file path in the configuration will be relative to this path."
        ),
    )

    llm_provider_name: str = Field(default="openai", description="The name of the LLM provider to be used.")
    write_output: bool = Field(default=True, description="Whether or not to write the outputs to a file.")
...
class BaseEvaluatorConfig(BaseConfig):
    ...
    answers_file: str = Field(default="answers.csv", description="Input CSV file with agents answers.")
    ...
    games_evaluations_file: str = Field(
        ...
        description=(
            "CSV file to write (or read) the evaluations of the pairwise games between agents answers to queries."
        ),
    )
    ...
    scoring_keys_answer_evaluator: List[str] = Field(
        default=["relevance"],
        description=(
            "When using answer_format=multi_field_json, the keys to extract from the answer for the answer evaluator."
        ),
    )
    scoring_keys_retrieval_evaluator: List[str] = Field(
        default=["relevance"],
        description=(
            "When using answer_format=multi_field_json, "
            "the keys to extract from the answer for the retrieval evaluator."
        ),
    )
...
class AllConfig(BaseEvaluatorConfig):
    reasoning_file: str = Field(
        default="reasonings.csv",
        description=(
            "CSV file to write (or read) the reasonings generated by the `Reasoner` retrieval evaluator "
            "on the relevance of the documents retrieved by the agents."
        ),
    )
    answers_evaluations_file: str = Field(
        default="answers_evaluations.csv",
        description=(
            "CSV file to write (or read) the evaluations of each agent's answers, generated by an `AnswerEvaluator`"
        ),
    )
    agents_evaluations_file: str = Field(default="agents_ranking.csv", description="Path to the output file")
