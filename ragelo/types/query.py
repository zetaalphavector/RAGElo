...
class Query(BaseModel):
    ...
    def add_retrieved_doc(
        ...
        if doc.did in self.retrieved_docs and not force:
            # Check if we are just adding that an agent has retrieved that document before
            if agent is not None and agent in doc.retrieved_by or agent is None:
                if agent is None:
                    logger.info(f"Query {self.qid} already have a document {doc.did} retrieved.")
                elif self.retrieved_docs[doc.did].text != doc.text:
                    logger.info(f"Query {self.qid} already have a document {doc.did} retrieved by agent {agent}.")
                return False
    ...
    def add_agent_answer(self, answer: AgentAnswer, force: bool = False) -> bool:
        ...
        if answer.agent in self.answers and not force:
            if self.answers[answer.agent].text == answer.text:
                return False
            logger.info(
                f"Answer from agent {answer.agent} already exists in query {self.qid} but with different text. Overwriting."
            )
    ...
    def get_qrels(
        ...
        if len(self.retrieved_docs) == 0:
            logger.info(f"Query {self.qid} does not have any retrieved documents. Returning empty qrels.")
            return {}
        non_judged_docs = 0
        ...
        if non_judged_docs > 0:
            logger.warning(f"Query {self.qid} has {non_judged_docs} documents not judged.")
        if non_judged_docs == len(self.retrieved_docs):
            logger.error(f"Query {self.qid} has no documents judged.")
        return qrels
