...
class Query(BaseModel):
    ...
    def add_agent_answer(
        self, answer: AgentAnswer, force: bool = False, exist_ok: bool = False
    ):
    ...
    def add_evaluation(
        ...
        if isinstance(evaluation, RetrievalEvaluatorResult):
            ...
            return True
        if (
            isinstance(evaluation, AnswerEvaluatorResult)
        ...
        if isinstance(evaluation, GroundednessEvaluatorResult):
            ...
            if self.answers[agent].groundedness_evaluation is not None and not force:
                logger.warning(
                    f"Agent {agent} in query {self.qid} already has a groundedness evaluation."
                )
                return False
            if self.answers[agent].groundedness_evaluation is not None:
                logger.info(
                    f"Agent {agent} in query {self.qid} already has a groundedness evaluation. Overwriting."
                )
    ...
    @classmethod
    def assemble_query(
        cls, query: Self | str, metadata: dict[str, Any] | None = None
    ) -> Self:
