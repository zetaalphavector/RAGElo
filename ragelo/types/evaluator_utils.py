...
from ragelo.types.types import AnswerEvaluatorTypes, RetrievalEvaluatorTypes, result_type_registry
...
def resolve_evaluator_result_type(
    ...
    if kind == "retrieval":
        try:
            return result_type_registry[f"retrieval:{RetrievalEvaluatorTypes(name_str)}"]
        except (ValueError, KeyError):
            raise ValueError(f"Unknown retrieval evaluator: {name_str}")
    if kind == "answer":
        try:
            return result_type_registry[f"answer:{AnswerEvaluatorTypes(name_str)}"]
        except (ValueError, KeyError):
