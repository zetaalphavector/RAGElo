"""
An Experiment is a collection of queries, and manages their retrieved documents, agent answers and their respective evaluations created by Evaluators.
...
class Experiment:
    ...
    Attributes:
        ...
        queries: (dict[str, Query]): A dictionary with the queries for this experiment. Can also be accessed directly with __getitem__.
        elo_tournaments: (list[EloTournamentResult]): A list with the results for Elo Tournaments played over this set of queries.
    Methods:
        add_query(query, query_id, doc_id, metadata, force): Adds a query (either a Query object or a string) to the current experiment. Returns the id of the added query.
        ...
        add_evaluation(evaluation, should_save, force): Adds an evaluation to a retrieved document, an answer or a pairwise game.
        evaluate_retrieval(metrics, relevance_threshold): Evaluates the retrieval performance of the agents using the specified metrics and the evaluations generated by a `RetrievalEvaluator`. This requires the ir_measures package to be installed.
        get_qrels(relevance_key, relevance_threshold, output_path, output_format): Gets the qrels (dictionary of relevance) for the queries. Documents with a score above relevance_threshold are considered relevant.  Optionally saves the result to output_path with output_format.
        get_runs(agents, output_path, output_format): Gets the runs (retrieval results) for the provided agents for all the queries. Optionally saves the result to output_path with output_format.
        ...
        add_retrieved_docs_from_runfile(run_file_path, corpus, top_k): Adds retrieved documents from a trec-formatted run file to the queries. Will create the Document objects as needed.
    ...
    def __init__(
        ...
        **data,
        ...
        2. If `csv_path` is not provided but `save_path` exists and points to a valid file, queries are loaded from the cache file.
        3. If both `csv_path` and `save_path` are provided, the method ensures that the queries loaded from the CSV file match those in the cache file.
    ...
    def add_query(
        ...
        Args:
            ...
            query_id (str | None, optional): The identifier for the query. If not provided, a default ID will be generated.
    ...
    def get_runs(
        ...
        Args:
            ...
            output_format (str): The format to save the runs. Defaults to "trec". If TREC, output_path should be a directory, and each agent will have a separate file.
        Returns:
