...
class Experiment:
    ...
    Methods:
        ...
        run_retrievers(retrievers, top_k, n_threads): Pools the documents each named `Retriever` returns
            for every query, recording the mapping key as the agent name. Skips pairs already pooled.
        ...
        compare_retrieval(agent_a, agent_b, metrics, ...): Paired per-metric comparison of two agents'
            runs, with win/tie/loss counts and a permutation-test p-value.
        ...
        save_result(result): Saves an evaluation result to the `evaluations_cache_path` file.
    ...
    def add_agent_answers(self, answers: list[AgentAnswer], force: bool = False, exist_ok: bool = False) -> int:
        ...
        for answer in answers:
            added += self.add_agent_answer(answer, force=force, exist_ok=exist_ok, should_save=False)
        if added > 0:
            self.save()
        if added < len(answers):
            logger.info(f"Added {added} out of {len(answers)} answers. Use force=True to overwrite existing answers.")
        return added
    ...
    def add_agent_answer(
        ...
        Args:
            ...
            should_save (bool): Whether to persist the experiment state after adding the answer.
                Defaults to True.
        """
        if query_id is None:
    ...
    def add_answers_from_multiple_csv(
        ...
        for f in answers_files:
            ...
            for line in csv.DictReader(p.read_text().splitlines()):
                ...
                if qid not in self.queries:
                    self.add_query(query, qid, should_save=False)
                self.add_agent_answer(answer, agent_name, qid, should_save=False)
                added_rows += 1
        if added_rows > 0:
            self.save()
