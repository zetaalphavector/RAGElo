...
class DomainExpertEvaluator(DocumentEvaluator):
    ...
    score_prompt = """
    Given the previous reasoning, please assign a score of 0, 1, or 2 to the retrieved document given the particular query. The score meaning is as follows:
    ...
    DOMAIN_SHORT = "like {domain_short}"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.domain_short = f" {domain_short}"
        self.extra_guidelines = extra_guidelines
        self.sys_prompt = self.sys_prompt.format(
            domain_long=domain_long,
            company_prompt_1=self.COMPANY_PROMPT_1.format(company) if company else "",
            company_prompt_2=self.COMPANY_PROMPT_2.format(company) if company else "",
            domain_short=self.DOMAIN_SHORT.format(domain_short) if domain_short else "",
        )
        ...
        for qid in self.queries:
            ...
            for did in self.documents[qid]:
                ...
                try:
                    ...
                    answer = self._process_first_answer(answer)
                except RetryError:
                    logger.warning(
                ...
                messages.append({"role": "assistant", "content": self.score_prompt})
                try:
                    answer = self.openai_client(messages)
    ...
    def _process_first_answer(self, answer: str) -> str:
        return answer
